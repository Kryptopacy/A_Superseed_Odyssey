...
import pygame
from src.config import TILE_SIZE, FPS
    ...
    def melee_attack(self, player):
        ...
        try:
            if player.inventory.has_sword:  # Ensure melee attack only works with the sword
                ...
                self.attacks.append(Attack(attack_rect, player.attack_power, player.facing_right, lifetime=5))
    ...
    def ranged_attack(self, player):
        ...
        try:
            if player.inventory.has_sword and player.ranged_attacks > 0:  # Ensure ranged attack only works with the sword
                ...
                self.attacks.append(Attack(attack_rect, player.attack_power, player.facing_right, dx=dx * 10, lifetime=50))
...
class Attack:
    def __init__(self, rect, power, facing_right, dx=0, lifetime=10):
